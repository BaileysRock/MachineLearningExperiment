...
def predictResult(method, FileName=""):
    ...
    if method == 0:
        ...
        theta = gradientDescent(X_train, Y_train, 0.01, 1e-7, 1e-7, 10000)
        plotScatter(X1Y0, X2Y0, X1Y1, X2Y1)
        drawTheta(-1, 5, theta, 0)
        predictPrecision(theta, X_test, Y_test)
        theta = newtonMethod(X_train, Y_train, 1e-7, 1e-7, 10000)
        plotScatter(X1Y0, X2Y0, X1Y1, X2Y1)
        ...
        X_train, X_test, Y_train, Y_test = TrainTestSplit(X, Y)
        theta = gradientDescent(X_train, Y_train, 0.001, 1e-7, 1e-7, 10000)
        plotScatter(X1Y0, X2Y0, X1Y1, X2Y1)
        drawTheta(-1, 5, theta, 0)
        predictPrecision(theta, X_test, Y_test)
        theta = newtonMethod(X_train, Y_train, 1e-7, 1e-7, 10000)
